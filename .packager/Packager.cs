...
foreach (var dir in dirs)
{
	...
	foreach (var (packageRoot, packageName, tomlTable) in packageRoots)
	{
		...
		c.WriteLine($"ℹ files after filtering: {filesList.Count}");
		{
			...
			var commitCount = 0;
			var tagCount = 0; // will be set to (amount of "v*" tags) + 1
			var branchName = "";
			...
			using (var repo = new Repository(gitRoot))
			{
				...
				if (headCommit != null)
				{
					...
					tagCount = allVersionTags.Length + 1;
					c.WriteLine($"ℹ tag count: {tagCount}");
					...
					// Get the (full linear history) pushed-only commit count
					// Get the remote tracking branch to count only pushed commits
					var trackingBranch = repo.Head.TrackedBranch;
					if (trackingBranch != null && trackingBranch.Tip != null)
					{
						// Count commits from the remote tracking branch's tip
						commitCount = repo.Commits
							.QueryBy(new CommitFilter { IncludeReachableFrom = trackingBranch.Tip })
							.Count();
					}
					else
					{
						// Fallback to all commits if no tracking branch
						commitCount = repo.Commits.Count();
					}
					c.WriteLine($"ℹ commit count: {commitCount}");
					// Get the current branch name
					branchName = repo.Head.FriendlyName;
					c.WriteLine($"ℹ branch name: {branchName}");
				}
			}
			...
			var filesOrdered = filesList
				...
				.ToArray();
			var filenamesHash = ComputeSHA256(Encoding.UTF8.GetBytes(string.Join("\n", filesOrdered)));
			...
			File.WriteAllText(Path.Combine(packageRoot, "Shared", "metadata_gen.lua"),
				...
				---@class {{Path.GetFileName(packageRoot)}}.metadata_gen
				...
				---@type {{Path.GetFileName(packageRoot)}}.metadata_gen
				return {
					...
					is_preview = {{(currentTag.Contains('-') ? "true" : "false")}},
					tag = "{{currentTag}}",
					owner = "{{repoOwner}}",
					repo = "{{repoName}}",
					path = "{{Path.GetRelativePath(gitRoot, packageRoot)}}",
					filenames_hash = "{{filenamesHash}}",
					files_hash = {{filesHashLua}},
					files = {
						{{string.Join("\n\t\t", filesOrdered.Select(static f => $"\"{f}\","))}}
					},
				}
				""".ReplaceLineEndings("\n").Trim() + "\n"
			);
